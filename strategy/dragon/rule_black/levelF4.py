...
class levelF4(base_level):
    ...
    def rule15(self):
        ...
        try:
            if t_limit(stock, data, 2):
                return False
            if not (data[-1].turnover > 2 * data[-2].turnover):
                return False
            for i in range(2):
                if not t_limit(stock, data, i):
                    return False
    ...
    def rule27(self):
        ...
        try:
            d = data[-1]
            if d.TP / weakenedIndex(self.shIndex, weak_degree=5) >= 30:
                return False
            if data[-1].TF >= 70:
                return False
    ...
    def rule31(self):
        ...
        try:
            ...
            if data[-2].turnover > 3 * (sum([_.turnover for _ in range5]) / 5):
                if data[-1].timeVol(timeStamp=data[-1].firstLimitTime) < data[-1].volume * 0.1:
                    return getMinute(stamp=data[-1].lastLimitTime) > '0945'
        except:
            pass
    ...
    def rule35(self):
        ...
        try:
            ...
            for i in range(3, 153):
                ...
                if t_limit(stock, data, i - 1):
                    continue
                if not data[-i - 1].close * 0.88 < data[-1].close:
                    continue
                if data[-i].turnover > 1.5 * data[-1].turnover:
                    if data[-i].high * 1.03 > data[-1].close:
                        if data[-i - 1].turnover > data[-1].turnover * 1.2:
    ...
    def rule37(self):
        ...
        try:
            ...
            badCount = 0
            for i in range(19):
                if move_avg(data, 10, i) < move_avg(data, 20, i):
                    badCount += 1
                if badCount >= 10:
                    return True
        except:
    ...
    def rule38(self):
        ...
        try:
            ...
            limitMinutePrev1 = prevMinute(limitMinute)
            limitMinutePrev2 = prevMinute(limitMinutePrev1)
            limitMinutePrev3 = prevMinute(limitMinutePrev2)
            if data[-1].timeVol(minute=limitMinute) < sum(
                    [data[-1].timeVol(minute=limitMinutePrev1),
                     data[-1].timeVol(minute=limitMinutePrev2),
                     data[-1].timeVol(minute=limitMinutePrev3)]):
                d = data[-1]
                if d.TF / weakenedIndex(self.shIndex) < 55:
                    if t_low_pct(data) < t_open_pct(data):
    ...
    def rule41(self):
        ...
        try:
            if model_1(self.stock, data):
                return False
            d = data[-1]
            if d.CP / weakenedIndex(self.shIndex, weak_degree=5) >= 55:
                return False
            if data[-1].TF >= 70:
                return False
            count1 = 0
    ...
    def rule43(self):
        ...
        try:
            ...
            if d.CP / weakenedIndex(self.shIndex, weak_degree=5) >= 60:
                return False
            if d.TF / weakenedIndex(self.shIndex, weak_degree=5) >= 60:
                return False
            if d.TP / weakenedIndex(self.shIndex, weak_degree=5) >= 35:
                return False
            for i in range(10, 101):
                if limit_height(stock, data, i) >= 3:
                    return False
        ...
        try:
            ...
            for i in range(40):
                ...
                if data[-i - 1].close < avg60:
                    badCount40 += 1
                if badCount5 >= 2 and badCount40 >= 3:
                    return True
        except:
            return False
    ...
    def rule48(self):
        ...
        try:
            ...
            for i in range(2, 152):
                ...
                if t_limit(stock, data, i - 1):
                    continue
                if data[-i - 1].close * 1.1 > data[-1].close:
    ...
    def rule49(self):
        ...
        stock = self.stock
        try:
            if not (data[-1].TF < 90 and data[-1].CP < 60):
                return False
            if t_limit(stock, data, 1):
                return False
            if model_1(stock, data):
            ...
            for i in range(2, 152):
                ...
                if t_limit(stock, data, i - 1):
                    continue
                nxt = data[-i]
                if data[-i - 1].close * 1.1 > highestClosePrice1to5:
                    if nxt.turnover > 1.6 * data[-1].turnover and data[-i - 1].turnover > data[-1].turnover * 1.2:
                        count += 1
                        if count >= 2:
                            return True
        except:
        ...
        try:
            ...
            for i in range(2, 152):
                ...
                if t_limit(stock, data, i - 1):
                    continue
                nxt = data[-i]
                if nxt.turnover > 2.3 * data[-i - 1].turnover:
                    if nxt.turnover > 3 * data[-1].turnover:
                        if nxt.high > 1.04 * data[-1].close:
                            if data[-i - 1].close < data[-1].close * 1.1:
                                return True
        except:
            pass
        ...
        try:
            ...
            for i in range(2, 152):
                ...
                if t_limit(stock, data, i - 1):
                    continue
                nxt = data[-i]
                if nxt.turnover > data[-1].turnover:
    ...
    def rule54(self):
        ...
        if data[-1].TF >= 70:
            return False
        ranges = data[-23:-3]
        high = max([_.high for _ in ranges])
        low = min([_.low for _ in ranges])
        return (high - low) / low > 0.4
            ...
            for i in range(153):
                ...
                if i in range(3, 153) and not flag:
                    ...
                    if t_limit(stock, data, i - 1):
                        continue
                    nxt = data[-i]
                    if data[-i - 1].turnover * 0.8 <= data[-1].turnover:
                        continue
                    if nxt.turnover <= 1.7 * d.turnover:
                        continue
                    if nxt.turnover <= 1.5 * data[-1].turnover:
                        continue
    ...
    def rule57(self):
        ...
        try:
            ...
            if t_limit(stock, data, 1):
                return False
            maxPriceIn1to5 = max([data[-i - 1].close for i in range(1, 6)])
            for i in range(2, 152):
                if not t_limit(stock, data, i):
                    continue
                if t_limit(stock, data, i - 1):
                    continue
                cur = data[-i - 1]
                if cur.turnover * 0.8 <= data[-1].turnover:
                    continue
                if cur.close * 1.1 > maxPriceIn1to5:
                    return True
        except:
            ...
    ...
    def rule58(self):
        ...
        try:
            ...
            for i in range(2, 152):
                ...
                nxt = data[-i]
                cur = data[-i - 1]
                if cur.turnover <= data[-1].turnover * 2.5:
                    continue
                if nxt.high * 1.04 > data[-1].close:
                    return True
        except:
    ...
    def rule59(self):
        ...
        try:
            ...
            if not getMinute(stamp=data[-1].firstLimitTime) < '0945':
                return False
            highest1to5 = max([data[-i - 1].close for i in range(1, 6)])
            for i in range(2, 152):
                if not t_limit(stock, data, i):
                    continue
                if model_1(stock, data, i):
                ...
                cur = data[-i - 1]
                nxt = data[-i]
                if not nxt.turnover > data[-1].turnover * 2.3:
                    continue
                if cur.close * 1.1 > highest1to5:
                    return True
        except:
            ...
    ...
    def rule60(self):
        ...
        try:
            ...
            for i in range(2, 152):
                ...
                nxt = data[-i]
                cur = data[-i - 1]
                if not nxt.turnover > data[-2].turnover * 2.3:
                    continue
                if not nxt.turnover > data[-1].turnover * 2.3:
                    continue
                if cur.close > highest2to6:
                    return True
        except:
            ...
