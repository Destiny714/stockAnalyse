...
def rule2(stock, data: List[dataModel]):
    ...
    if not model_1(stock, data, 1):
        return False
    if data[-1].volume() > 2.5 * data[-2].volume():
        return True
...
def rule7(stock, data: List[dataModel]):
    if not model_1(stock, data, 1):
        return False
    range10 = data[-12:-2]
    if data[-2].volume() > 0.25 * max([_.volume() for _ in range10]):
        return True
...
def rule8(stock, data: List[dataModel]):
    ...
    if not t_limit(stock, data, 2):
        return False
    if data[-2].volume() > 2 * data[-3].volume():
        if data[-1].volume() > 2 * data[-2].volume():
            if not model_1(stock, data, 1):
                return True
...
def rule12(stock, data: List[dataModel]):
    ...
    if not model_1(stock, data):
        return False
    if data[-1].volume() > 0.5 * data[-2].volume():
        return True
...
def rule13(data: List[dataModel]):
    ...
    for i in range(1, 19):
        ...
        t2 = data[-i - 1]
        t3 = data[-i - 2]
        if t3.volume() < t2.volume() < t1.volume():
