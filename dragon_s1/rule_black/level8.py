...
def rule20(data: List[dataModel], virtual):
    gemData = None
    for i in range(2):
        if data[-i - 1].limitOpenTime() <= 2:
            continue
        gemData = collectData('399006', dateRange=5,
                              aimDate=data[-1 if virtual is None else -2].date()) if gemData is None else gemData
